...
3. **Product-Derived Lighting Plan**: Before writing scene prose, infer the light from the product and usage context. This is an internal reasoning step, not a rigid label to paste into the final prompt.
   ...
   - **Color protection**: Keep the source product's body color, material color, and brand color atmosphere unchanged. Accent light may separate the product from the background, but it must not recolor the product surface.
...
7. **Brand Effect Is Protected**: The EXCITAT shelf/shard is a required premium brand element, not pollution. Keep a category-colored rim glow, layered plate/shard shape, subtle cast shadow, and bold EXCITAT wordmark. Tech energy styling may live inside the brand shelf for tech products only; it must not become the background style.
8. **Pollution Cleanup**: Do not inherit manual prompt artifacts from another product test. Remove unrelated side-light line defaults, fixed corner badge defaults, and generic graphic-effect styling unless the current product physically creates that light or energy.
9. **Impact Without Drift**: Increase visual impact through product scale, crisp silhouette separation, shallow depth, strong but clean value hierarchy, contact shadows, category-colored value islands, and the protected brand shelf/shard. Keep all added effects physically relevant to the product and usage scene.
...
7. **Parameter Layout Role**: If the planner selected a hero parameter, reserve a first-read slab/circle in the top-left or left-center. If it selected secondary parameters, reserve a side/lower-left rail. If it selected part callouts, they must explain a real buyer proof point without competing with the product body.
...
9. **Protected Product Zone**: Treat the product body as a protected layer, the same way product color is protected from lighting. Before writing overlay copy, separate the layout into a protected product zone and an outside information zone. Text chips, circular labels, badges, value islands, and readable callout labels must stay in the outside information zone; that zone may be a clean pocket directly beside the product edge. A callout may touch the product only with a tiny anchor dot or very short elbow pointer. If there is no clean outside space for the readable label, omit that callout from the main image and move the fact to a detail image.
10. **Depth & Shadows**: Avoid floatation unless it fits a tech product. Use product-matched contact shadows, ambient depth, rim separation, reflection, or glow only when the scene has a physical reason for it.
11. **Product Color Atmosphere**: Lighting must support the product's original color mood. White appliances stay clean white; warm wood or fabric stays warm and tactile; black electronics can use controlled rim separation. Avoid lighting that makes the product look like a different SKU.
